...
ifeq ($(BR2_AW_WESTON),y)
WESTON_SITE_METHOD = local
WESTON_SITE = $(TOPDIR)/../../platform/framework/weston
else
...
WESTON_CONF_OPTS += --enable-egl
WESTON_CONF_OPTS += --enable-simple-egl-clients
WESTON_DEPENDENCIES += sunxi-mali
else
WESTON_CONF_OPTS += \
	--disable-egl \
	--disable-simple-egl-clients
...
ifeq ($(BR2_PACKAGE_WESTON_IVI_SHELL),y)
WESTON_INI_FILE = $(TOPDIR)/../../platform/framework/weston/ivi-shell/weston.ini.in
else
WESTON_INI_FILE = package/weston/weston.ini
endif

define WESTON_INSTALL_INIT_SYSV
	$(INSTALL) -D -m 0755 $(WESTON_INI_FILE) \
                $(TARGET_DIR)/etc/xdg/weston.ini
	$(INSTALL) -D -m 0755 package/weston/S90weston \
		$(TARGET_DIR)/etc/init.d/S90weston

	mkdir -p $(STAGING_DIR)/usr/include/weston/protocol
	cp -rf $(@D)/protocol/*.h $(STAGING_DIR)/usr/include/weston/protocol
endef
