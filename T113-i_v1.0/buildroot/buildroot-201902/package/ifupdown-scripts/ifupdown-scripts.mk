...
IFACE0 = $(shell echo "$(IFUPDOWN_SCRIPTS_DHCP_IFACE)" | awk -F',' '{print $$1}')
IFACE1 = $(shell echo "$(IFUPDOWN_SCRIPTS_DHCP_IFACE)" | awk -F',' '{print $$2}')

ifneq ($(IFACE0),)
define IFUPDOWN_SCRIPTS_DHCP0
	( \
		echo ; \
		echo "auto $(IFACE0)"; \
		echo "iface $(IFACE0) inet dhcp"; \
...
ifneq ($(IFACE1),)
define IFUPDOWN_SCRIPTS_DHCP1
	( \
		echo ; \
		echo "auto $(IFACE1)"; \
		echo "iface $(IFACE1) inet dhcp"; \
		echo "  pre-up /etc/network/nfs_check"; \
		echo "  wait-delay 15"; \
		echo "  hostname \$$(hostname)"; \
	) >> $(TARGET_DIR)/etc/network/interfaces
...
define IFUPDOWN_SCRIPTS_INSTALL_TARGET_CMDS
	...
	$(IFUPDOWN_SCRIPTS_DHCP0)
	$(IFUPDOWN_SCRIPTS_DHCP1)
endef
