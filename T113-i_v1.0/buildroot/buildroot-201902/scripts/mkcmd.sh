...
function init_defconf()
{
	...
	defconf=`awk '$1=="'$pattern'" {print $2,$3}' buildroot-201611/scripts/mkrule`
	if [ -n "${defconf}" ] ; then
        ...
        if [ -z "${choice//[0-9]/}" ] ; then
            if [ $choice -ge 0 -a $choice -lt $cnt ] ; then
                ...
                if [ -n "`echo ${LICHEE_CHIP} | grep "sun5[0-9]i"`" ]; then
                        ...
                        echo "export LICHEE_ARCH=arm64" >> .buildconfig
                else
                        export LICHEE_ARCH="arm"
                        echo "export LICHEE_ARCH=arm" >> .buildconfig
                fi
                break
            fi
        fi
...
function prepare_toolchain()
{
    ...
    tooldir=${LICHEE_BR_OUT}/host/opt/ext-toolchain
    if [ ! -d ${tooldir} ] ; then
