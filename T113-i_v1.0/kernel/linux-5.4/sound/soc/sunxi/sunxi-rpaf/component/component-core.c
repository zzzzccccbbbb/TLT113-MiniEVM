...
static int snd_soc_rpaf_misc_common_set_algo_params(struct snd_dsp_component *dsp_component,
		void __user *arg)
{
	struct snd_soc_rpaf_info *rpaf_info = dsp_component->rpaf_info;
	struct msg_component_package *msg_component = &dsp_component->msg_component;
	struct snd_soc_dsp_component *soc_component = &msg_component->soc_component;
	struct snd_soc_dsp_algo_params_transfer __user *_algo_params = arg;
	struct snd_soc_dsp_algo_params_transfer *algo_params = &soc_component->algo_params;
	int ret = 0;

	if (copy_from_user(algo_params, _algo_params, sizeof(*algo_params))) {
		dev_err(rpaf_info->dev, "cannot copy algo_params.\n");
		return -EFAULT;
	}

	soc_component->comp_mode = SND_DSP_COMPONENT_MODE_ALGO;
	soc_component->cmd_val = SND_SOC_DSP_COMPONENT_ALGO_PARAMS_SET;
	/* 发送给msgbox to dsp */
	ret = sunxi_hifi_component_block_send(rpaf_info->dsp_id, msg_component);
	if (ret < 0) {
		pr_err("%s line:%d error:%d\n", __func__, __LINE__, ret);
				      ...
				      void __user *arg)
{
	...
	case SND_SOC_DSP_COMPONENT_IOCTL_STOP:
		...
		put_user(result, (int __user *)arg);
		break;
	case SND_SOC_DSP_COMPONENT_IOCTL_ALGO_PARAMS_SET:
		result = snd_soc_rpaf_misc_common_set_algo_params(dsp_component, arg);
		break;
	/* 接口用于dump，暂时不用 */
