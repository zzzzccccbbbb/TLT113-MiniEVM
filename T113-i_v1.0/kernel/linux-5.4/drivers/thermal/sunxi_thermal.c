...
#define SUN8IW20_CHIP_ID_OFF		(0x00)
				   ...
				   u16 *caldata, int callen)
{
	struct device *dev = tmdev->dev;
	int i, ft_temp, t0, t1;
	unsigned int cp_ver, aldo_cal, chip_id;

	sunxi_get_module_param_from_sid(&chip_id, SUN8IW20_CHIP_ID_OFF, 4);
	chip_id &= 0xffff;
	dev_info(dev, "sun8iw20 chip id: %d\n", chip_id);
	...
	for (i = 0; i < tmdev->chip->sensor_num; i++) {
		 ...
		 * cdata = 0x800 - [(ft_temp - T) * 1000 / scale]
		 */
		delta = (ft_temp * 100 + t1 - sunxi_ths_reg2temp(tmdev, reg) - t0)
			/ tmdev->chip->scale;
		cdata = CALIBRATE_DEFAULT - delta;
		if (cdata & ~TEMP_CALIB_MASK) {
			dev_warn(dev, "sensor%d is not calibrated.\n", i);
