...
static void geth_adjust_link(struct net_device *ndev)
{
	...
	if (phydev->link) {
		...
		/* kernel-5.4 not PHY_FORCING defined! */
...
static int geth_hw_init(struct platform_device *pdev)
{
	...
	if(!of_property_read_u32(np, "phy-tx-delay", &value))
		phy_tx_delay = value;

	if(!of_property_read_u32(np, "phy-rx-delay", &value))
		phy_rx_delay = value;

	/* config pinctrl */
	if (EXT_PHY == priv->phy_ext) {
		priv->phyrst = of_get_named_gpio_flags(np, "phy-rst", 0, (enum of_gpio_flags *)&cfg);
		priv->rst_active_low = (cfg.data == OF_GPIO_ACTIVE_LOW) ? 0 : 1;
