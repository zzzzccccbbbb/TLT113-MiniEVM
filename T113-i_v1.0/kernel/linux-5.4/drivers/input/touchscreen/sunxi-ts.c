...
static void sunxi_ts_hwinit(struct sunxi_ts *chip)
{
	...
	*/
	...
	writel(ADC_FIRST_DLY(1) | ADC_FIRST_DLY_MODE(1) | ADC_CLK_DIV(2) | FS_DIV(6) | T_ACQ(0x3f),
	       chip->base + TP_CTRL0);
#else
	/* writel(0x0fa7004f, chip->base + TP_CTRL0); */
	writel(ADC_FIRST_DLY(1) | ADC_FIRST_DLY_MODE(1) | ADC_CLK_DIV(2) | FS_DIV(7) | T_ACQ(0x3f),
	       chip->base + TP_CTRL0);
#endif
	/*
	...
	writel(TP_SENSITIVE_ADJUST(ts_config->tp_sensitive_adjust) | TP_MODE_SELECT(0),
	       chip->base + TP_CTRL2);
#else
	writel(0xc0800fff, chip->base + TP_CTRL2);
...
#else
	writel(0x00000004, chip->base + TP_CTRL3);
#endif
	/*
	...
	*/
#if ORIGIN
	reg_val = STYLUS_UP_DEBOUN(5) | STYLUS_UP_DEBOUN_EN(1) | BIT(chip->hwdata->chopper_en_bitofs) |
		  BIT(chip->hwdata->tp_mode_en_bitofs);
	writel(reg_val, chip->base + TP_CTRL1);
#else
	writel(0x00010121, chip->base + TP_CTRL1);
#endif
}
