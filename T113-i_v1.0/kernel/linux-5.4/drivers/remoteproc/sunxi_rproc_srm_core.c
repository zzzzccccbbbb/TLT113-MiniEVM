...
/*
* linux/drivers/remoteproc/sunxi_rproc_srm_core.c
*
...
#define SUNXI_RPROC_SRM_CORE_VERSION "V1.0.1"
			...
			void *priv, u32 src)
{
	...
	dev_dbg(&rpdev->dev, "(%s:%d) will call chain\n", __func__, __LINE__);
...
static int rpmsg_srm_probe(struct rpmsg_device *rpdev)
{
	...
	p.message_type = RPROC_SRM_MSG_INIT_TRIGGER;
	dev_dbg(&rpdev->dev, "(%s:%d)\n", __func__, __LINE__);

	/* Send an empty message to complete the initialization */
	ret = rpmsg_send(rpdev->ept, (void *)&p, sizeof(p));
	if (ret)
...
static int rproc_srm_core_prepare(struct rproc_subdev *subdev)
{
	...
	/* trigger all rproc srm dev probe */
	ret = devm_of_platform_populate(dev);
	if (ret) {
		dev_err(dev, "cannot populate node (%d)\n", ret);
		return ret;
	}
	dev_dbg(dev, "(%s:%d) trigger all rproc srm dev probe ok\n", __func__, __LINE__);
...
static int rproc_srm_core_probe(struct platform_device *pdev)
{
	...
	dev_info(dev, "sunxi rproc srm core driver %s\n", SUNXI_RPROC_SRM_CORE_VERSION);
...
MODULE_DESCRIPTION("Allwinnertech Remoteproc System Resource Manager driver - core");
