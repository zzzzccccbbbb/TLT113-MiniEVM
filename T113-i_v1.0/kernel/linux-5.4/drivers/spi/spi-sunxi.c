				 ...
				 struct spi_transfer *xfer)
{
	...
	return (sspi->use_dma && xfer->len > BULK_DATA_BOUNDARY);
}
...
/* request dma channel */
static int sunxi_spi_request_dma(struct spi_master *master)
{
	...
	if (IS_ERR(master->dma_tx)) {
		SPI_ERR("[spi%d] Can't request DMA tx channel\n", master->bus_num);
	...
	if (IS_ERR(master->dma_rx)) {
		SPI_ERR("[spi%d] Can't request DMA rx channel\n", master->bus_num);
		ret = PTR_ERR(master->dma_rx);
		goto err1;
	}

	dprintk(DEBUG_INIT, "[spi%d] Request DMA channel %s (tx) and %s (rx) for DMA transfers\n",
		master->bus_num, dma_chan_name(master->dma_tx), dma_chan_name(master->dma_rx));
...
static int sunxi_spi_release_dma(struct spi_master *master)
{
	if (master->dma_tx)
		dma_release_channel(master->dma_tx);
	if (master->dma_rx)
...
static int sunxi_spi_xfer(struct spi_device *spi, struct spi_transfer *t)
{
	...
	case QUAD_HALF_DUPLEX_RX:
	{
		...
		if (sunxi_spi_can_dma(spi->master, spi, t)) {
					...
					struct spi_transfer *t)
{
	...
	/*
	 * When poll the end of spi data transfer,
	 * the hardware interrupt of spi controller is needed.
	 * But the linux handler of spi controller interrupt
	 * should not be called.
	 *
	 * So, when spi_message.is_poll is set, the spi data
	 * transfer end is polled by cpu. Call disable_irq(sspi->irq)
	 * to disable spi controller interrupt handler
	 * before spi transfer working.
	 *
	 * When spi_message.is_poll is reset, the spi data
	 * transfer end is detected by irq controller. Call
	 * enable_irq(sspi->irq) to enable spi controller
	 * interrupt handler before spi transfer working.
	 */
	if (t->is_poll)
	...
	/*
	* Read the spi interrupt status register to get the data transfer
	* completed state or data transfer error state.
	* The spi interrupt is the real end of one data transfer, not the
	* tx/rx fifo empty.
	*/
	if (t->is_poll) {
		spin_lock_irqsave(&sspi->lock, flags);
		while (!(status & SPI_INT_STA_ERR) && !(status & SPI_INT_STA_TC)) {
			status = spi_qry_irq_pending(base_addr);
		}
		if (status & SPI_INT_STA_ERR || !(status & SPI_INT_STA_TC)) {
			sspi->mode_type = MODE_TYPE_NULL;
			spi_disable_irq(SPI_INT_STA_TC | SPI_INT_STA_ERR, base_addr);
			spi_soft_reset(base_addr);
			SPI_ERR("cpu receive data error!, status:0x%x\n", status);
			return -1;
		}
...
#define SPI_FIFO_SIZE (64)
	...
	*/
	...
	if (tcnt) {
		...
		/* >64 use DMA transfer, or use cpu */
		if (!sunxi_spi_can_dma(spi->master, spi, tx)) {
			xcnt = 0;
			poll_time = 0xfffff;
	...
	if (rcnt) {
		if (!sunxi_spi_can_dma(spi->master, spi, rx)) {
			xcnt = 0;
			poll_time = 0xfffff;
...
#ifdef CONFIG_SPI_SUNXI_ATOMIC_XFER
...
static int sunxi_spi_resource_get(struct sunxi_spi *sspi)
{
	...
	ret = sunxi_spi_request_dma(sspi->master);
	if (ret) {
		SPI_ERR("dma channel request failed, or no dma comfiguration information in dts\n");
		sspi->use_dma = false;
	} else {
		sspi->use_dma = true;
	}
...
#if IS_ENABLED(CONFIG_DMA_ENGINE)
			sspi->use_dma ? dma_chan_name(master->dma_tx) : "NULL",
			sspi->use_dma ? dma_chan_name(master->dma_rx) : "NULL",
#endif
