...
#define SUN8IW20_SPEED_MASK    GENMASK(15, 0)
#define SUN20IW1_SPEED_MASK    GENMASK(15, 0)
...
static void sun8iw20_bin_xlate(bool bin_select, bool high_speed, char *name, u32 nv_bin, u32 *versions)
{
	int value = 0;
	...
	if (bin_select) {
		if (high_speed) {
			if (bin <= 1)
				...
				pr_err("%s:%d: unsupport efuse bin:%d\n", __func__, __LINE__, bin);
				*versions = 0b10000000;
			}
		} else {
			if (bin <= 1)
		...
		if (high_speed)
			...
			pr_err("%s:%d: undefine specification\n", __func__, __LINE__);
			*versions = 0b10000000;
		}
		snprintf(name, MAX_NAME_LEN, "b%d", value);
	}
...
{
	switch (ver_data.nv_speed & SUN8IW20_SPEED_MASK) {
	case 0x6000:
		*versions = 0b0010;
		sun8iw20_bin_xlate(false, true, name, ver_data.nv_bin, versions);
		break;
	case 0x7c00:
		*versions = 0b0001;
		sun8iw20_bin_xlate(true, false, name, ver_data.nv_bin, versions);
		break;
	case 0x6400:
	case 0x7000:
	case 0x2000:
	default:
		*versions = 0b0001;
		sun8iw20_bin_xlate(true, true, name, ver_data.nv_bin, versions);
		break;
	}
...
static void sun20iw1_bin_xlate(bool bin_select, bool high_speed, char *name, u32 nv_bin, u32 *versions)
{
	int value = 0;
	u32 bin = (nv_bin >> 12) & 0xf;
...
{
	switch (ver_data.nv_speed & SUN20IW1_SPEED_MASK) {
	case 0x5c00:
	case 0x5e00:
		*versions = 0b0010;
		sun20iw1_bin_xlate(false, false, name, ver_data.nv_bin, versions);
		break;
	case 0x7400:
		*versions = 0b0001;
		sun20iw1_bin_xlate(false, true, name, ver_data.nv_bin, versions);
		break;
	case 0x5000:
	default:
		*versions = 0b0001;
		sun20iw1_bin_xlate(true, false, name, ver_data.nv_bin, versions);
		break;
	}
