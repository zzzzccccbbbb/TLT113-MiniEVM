			   ...
			   int total)
{
	...
	dev_dbg(&rpdev->dev, "%s: total = %d, msg_size = %d\n",
			__func__, total, msg_size);
...
static int __init rpmsg_tty_init(void)
{
	...
	pr_info("%s: Sunxi rpmsg tty driver will init\n", __func__);
	rpmsg_tty_driver = tty_alloc_driver(MAX_TTY_RPMSG_INDEX, 0);
	if (IS_ERR(rpmsg_tty_driver))
		return PTR_ERR(rpmsg_tty_driver);

	rpmsg_tty_driver->driver_name = "tty_rpmsg";
	rpmsg_tty_driver->name = "tty_rpmsg";
	...
	if (!err) {
		pr_info("%s: Sunxi rpmsg tty driver init ok\n", __func__);
		return 0;
	}
...
MODULE_DESCRIPTION("Allwinnertech virtio remote processor messaging tty driver");
