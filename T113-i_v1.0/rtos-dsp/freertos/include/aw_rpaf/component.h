...
/**
 * each algorithm should use it to transfer algorithm params from
 *      linux user space -> linux kernel space -> dsp.
 * linux kernel space and dsp transfer data by using shared memory, the size of data is limited by
 * msgbox(RPC:Remote Procedure Call),which has defined message buffer size.
 */
struct snd_soc_dsp_algo_params_transfer {
	enum SND_DSP_COMPONENT_TYPE algo_type;
	/* the data[64] maybe not enough to transfer all the params.Therefore,the params maybe devided
	 * into several snd_soc_dsp_algo_params_transfer, and it will be set "true" when it  was the
	 * last snd_soc_dsp_algo_params_transfer.
	 */
	bool is_changed;
	/*
	 * the size of effective data;
	 */
...
/**
 * the data source is *snd_soc_dsp_algo_params_transfer.
 * process in *snd_dsp_hal_component_process_driver->prepare,so the algo_type is useless.
 * the data is complete algorithm params, which is different with *snd_soc_dsp_algo_params_transfer->data.
 */
struct snd_soc_dsp_algo_params {
	/*
	 * to notify *snd_dsp_hal_component_process_driver->create to effect the data.
	 */
	bool is_changed;
	void* data;
};
...
struct snd_soc_dsp_component {
	...
	struct snd_soc_dsp_algo_params_transfer algo_params;
