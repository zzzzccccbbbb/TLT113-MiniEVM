...
#include "../drivers/video/sunxi/disp2/disp/de/bsp_display.h"
...
int env_load(void)
{
	...
	if (work_mode == WORK_MODE_BOOT) {
		...
		for (screen_id = 0; screen_id < num_screens; screen_id++) {
			...
			if (mgr) {
				...
				if (dispdev) {
					if (mgr->device->type == DISP_OUTPUT_TYPE_LCD) {
						while ((lcd_op_finished[0] == 0) && timeout--) {
							udelay(1000);
						}
					} else {
						while (!dispdev->is_enabled(dispdev) && timeout--) {
							udelay(1000);
						}
					}
				}
			...
			if (timeout <= 0) {
				tick_printf("%s:%s:%d:Waiting for display %d timeout\n",
					    __FILE__, __func__, __LINE__,mgr->device->type);
			}
		}
	}
#endif
