...
static int auto_update_system(void)
{
	int nRet=0;
	int if_need_auto_force_burn=0;
	int workmode = get_boot_work_mode(); /* uboot_spare_head.boot_data.work_mode; */
	nRet = script_parser_fetch("/soc/target", "auto_usb_ota", &if_need_auto_force_burn, 0);
	printf("=====YTR==workmode=%d==if_need_auto_force_burn=%d, nRet=%d,%s\n", workmode, if_need_auto_force_burn, nRet, __func__);

	if ( workmode == WORK_MODE_BOOT && 1 == if_need_auto_force_burn)
	{
		run_command("auto_update_check", 0);
		printf("===========YTR===update system ok==next reboot system==\n");
		/* run_command("reset", 0); */
	}
	return 0;
}
...
void board_init_r(gd_t *new_gd, ulong dest_addr)
{
	...
	/* SOM-TLT113 led2: PC1 */
	/* Indicate that the u-boot program starts running */
	/* Set led2 gpio function output mode */
	reg_val = readl(PC_CFG0_ADDR);
	reg_val &= ~(0xf << 4);
	reg_val |= (0x1 << 4);
	writel(reg_val, PC_CFG0_ADDR);

	/* Set led2 gpio output high */
