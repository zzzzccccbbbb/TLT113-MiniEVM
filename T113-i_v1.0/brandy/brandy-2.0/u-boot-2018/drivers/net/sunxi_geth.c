...
static u32 phy_tx_delay = 0, phy_rx_delay = 0;
//#define PKT_DEBUG
#ifdef PKT_DEBUG
...
/* it should be defined in uboot/include/configs/sun?iw?p?.h */
	...
	if (used_type == INT_PHY) {
		...
		/* config PIO for gmac */
		geth_nodeoffset = fdt_path_offset(working_fdt, "/soc/eth@4500000");
		...
		/* get phy-tx-delay for yt8521sh */
		if (fdt_getprop_u32(working_fdt, geth_nodeoffset, "phy-tx-delay", &phy_tx_delay) < 0) {
			printf("get phy-tx-delay fail!");
			return -1;
		}
		/* get phy-rx-delay for yt8521sh */
		if (fdt_getprop_u32(working_fdt, geth_nodeoffset, "phy-rx-delay", &phy_rx_delay) < 0) {
			printf("get phy-rx-delay fail!");
			return -1;
		}
	}
...
static int mii_phy_init(struct eth_device *dev)
{
	...
	/* Read rgmii cfg1 */
	geth_phy_write(dev, phy_addr, YT8521SH_EXT_REG_ADDR_OFFSET_REG, YT8521SH_EXT_COMMON_RGMII_CFG1);
	mdelay(100);
	value = geth_phy_read(dev, phy_addr, YT8521SH_EXT_REG_DATA_REG);
	mdelay(100);

	value &= ~(0xf << YT8521SH_EXT_REG_TX_OFFSET);
	value |= (phy_tx_delay << YT8521SH_EXT_REG_TX_OFFSET);

	value &= ~(0xf << YT8521SH_EXT_REG_RX_OFFSET);
	value |= (phy_rx_delay << YT8521SH_EXT_REG_RX_OFFSET);

	/* Config phy tx/rx delay */
	geth_phy_write(dev, phy_addr, YT8521SH_EXT_REG_ADDR_OFFSET_REG, YT8521SH_EXT_COMMON_RGMII_CFG1);
	mdelay(100);
	geth_phy_write(dev, phy_addr, YT8521SH_EXT_REG_DATA_REG, value);
