...
int sunxi_update_bootcmd(void)
{
	...
	if (storage_type == STORAGE_SD) {
		sunxi_str_replace(boot_commond, "setargs_nand", "setargs_sd");
		debug("bootcmd set setargs_sd\n");
	} else if (storage_type == STORAGE_EMMC) {
		sunxi_str_replace(boot_commond, "setargs_nand", "setargs_emmc");
		debug("bootcmd set setargs_emmc\n");
