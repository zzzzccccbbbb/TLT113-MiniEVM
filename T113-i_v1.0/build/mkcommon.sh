...
if [ "x${LICHEE_LINUX_DEV}" = "xlongan" -o "x${LICHEE_LINUX_DEV}" = "xtinyos" \
	-o "x${LICHEE_LINUX_DEV}" = "xbsp" ] ; then
	if [ "x${LICHEE_BUILDING_SYSTEM}" = "x" -o "x${LICHEE_BR_VER}" = "x" ];then
		echo "nor flash, skip buildroot"
		SKIP_BR=1;
	else
		SKIP_BR=0;
	fi
