...
"
    ...
    printf "Swupdate:
    swupdate_pack_swu	- pack for swupdate
"
    return 0
}
...
function build_help()
{
    ...
    cqt                 - Changes directory to the qt
    cgcc                - Changes directory to the cross compiler paths
"
    build_usage
    pack_usage
...
function cqt()
{
	cd $LICHEE_TOP_DIR/platform/framework/qt/qt-everywhere-src-5.12.5/
}

function cgcc()
{
	cd $LICHEE_TOOLCHAIN_PATH/bin
}
...
function ota_ab(){
	...
	cp ${LICHEE_BOARD_CONFIG_DIR}/${LICHEE_LINUX_DEV}/sys_partition_ab.fex  ${LICHEE_BOARD_CONFIG_DIR}/${LICHEE_LINUX_DEV}/sys_partition.fex -rf
}
